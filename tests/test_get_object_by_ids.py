...
def test_get_object(make_call: APIRequestContext) -> None:
    ...
    id_1 = load_test_data("schemas/get_objects_by_id.yaml")["id_1"]
    id_2 = load_test_data("schemas/get_objects_by_id.yaml")["id_2"]
    id_3 = load_test_data("schemas/get_objects_by_id.yaml")["id_3"]
    endpoint = get_endpoint("endpoint_objects")
    get_object = make_call.get(endpoint + f"?id={id_1}&id={id_2}&id={id_3}")

    assert get_object.ok
    get_response = get_object.json()

    # Load the schema from file
    schema = load_test_data("schemas/get_objects_by_id.yaml")["schema"]
