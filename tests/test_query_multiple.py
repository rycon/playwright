...
def test_query_media_anime(gql_query: APIRequestContext) -> None:
    ...
    payload = load_test_data("/graphql/query_multiple.yaml")[0]["payload"]
    variables = load_test_data("/graphql/query_multiple.yaml")[0]["variables"]


    query = gql_query.post("/", data={"query": payload, "variables":variables})
    assert query.ok

    expected = load_test_data("/graphql/query_multiple.yaml")[0]["response"]
